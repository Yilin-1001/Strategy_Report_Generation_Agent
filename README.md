...
> **页面说明：** 开始页用于输入战略报告主题。页面概览诊断建模、战略推演和协同审核三项核心能力；提交主题后，系统将自动创建报告任务并进入多智能体生成流程。

**准备阶段 - 章节状态初始化**

<img src="docs/screenshots/准备阶段.png" width="720"/>

> **页面说明：** 准备阶段为当前章节建立上下文与任务状态。左侧显示章节目录、处理状态和总体进度，顶部展示各 Agent 节点的执行位置，右侧提供批准、补充数据、重新分析、重写和终止报告等人工控制入口。
...
> **页面说明：** Researcher Agent 根据章节目标执行多查询知识库检索，并以卡片形式展示命中文档、来源路径、内容片段和相关度。检索结果将作为后续结构化分析与报告引用的事实依据，便于核查信息来源。

**分析过程 - 战略模型驱动分析**

<img src="docs/screenshots/分析过程.png" width="720"/>

> **页面说明：** Analyst Agent 将检索证据映射到当前章节采用的战略模型。图中以 SWOT 为例，按优势、劣势、机会和威胁提取关键事实，保留对应文档编号，并进一步归纳洞察要点。

**内容撰写 - 章节正文生成**

<img src="docs/screenshots/内容撰写.png" width="720"/>

> **页面说明：** Writer Agent 综合检索材料、分析结果和已审核的战略蓝图生成章节草稿。正文按战略维度组织论点、目标与行动建议，并在关键结论后标注来源；页面两侧同步展示章节进度和人工修订操作。

**AI 评审详情 - 六维质量评估**

<img src="docs/screenshots/AI评审详情.png" width="720"/>

> **页面说明：** 独立评审模型从模型运用、数据支撑、逻辑结构、洞察深度和写作规范等维度评估草稿，给出总分、分项反馈、通过建议及具体问题。人工审核者可据此批准内容，或要求补充数据、重新分析和重写。
...
> **页面说明：** 系统汇总前三章的诊断结果，形成包含企业使命、SWOT、TOWS 战略组合和 KPI 的战略蓝图。蓝图经人工审核后将成为后续章节的统一约束，确保战略目标、举措和指标前后一致。

**报告整体 - 多章节完整输出**

<img src="docs/screenshots/报告整体.png" width="720"/>

> **页面说明：** 最终报告页整合执行摘要、战略蓝图和全部章节成果。左侧目录支持快速定位章节，正文集中呈现关键判断、战略目标、实施举措与量化指标，便于整体审阅和后续导出使用。
